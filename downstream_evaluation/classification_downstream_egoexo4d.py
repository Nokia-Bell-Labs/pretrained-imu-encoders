...
def eval_downstream(configs, args):
    ...
    global IMU_SAMPLING_RATE, EMBED_DIM, NUM_CLASSES, BATCH_SIZE, NUM_EPOCHS, TEXT_PROMPTS
    ...
    fewshot_model = FewShotModel(imu_encoder=encoder, clf=clf, T_max=int(max(len(train_dataset) / BATCH_SIZE, 1) * NUM_EPOCHS), finetune=args.finetune, num_classes=NUM_CLASSES, emb_type=args.emb_type, imagebind=args.imagebind)
