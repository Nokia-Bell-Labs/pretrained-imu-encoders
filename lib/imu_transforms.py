...
# @TODO: Implement with shape (batch, channels, time) instead of (batch, time, channels)
...
def main():
    ...
    X = np.random.rand(5, 6, 250)

    # Create a 1, 250, 6 array with each channel increasing
    # X = np.concatenate([np.arange(4)[np.newaxis, :, np.newaxis], np.arange(4)[np.newaxis, :, np.newaxis] + 1, np.arange(4)[np.newaxis, :, np.newaxis] + 2, np.arange(4)[np.newaxis, :, np.newaxis] + 3, np.arange(4)[np.newaxis, :, np.newaxis] + 4, np.arange(4)[np.newaxis, :, np.newaxis] + 5, np.arange(4)[np.newaxis, :, np.newaxis]+6, np.arange(4)[np.newaxis, :, np.newaxis]+7, np.arange(4)[np.newaxis, :, np.newaxis]+8], axis=-1)
    # print(X)
    plt.figure()
    for i in range(4):
        plt.plot(X[0, i, :])
    ...
    X_aug = rotation_transform_vectorized(X)
    plt.figure()
    for i in range(4):
        plt.plot(X_aug[0, i, :])
